...
echo PROMETHEUS_ALERTS_ROUTE=\"$(oc get route --namespace='prometheus' -o go-template --template='{{.spec.host}}' alerts 2> /dev/null)\"
echo PROMETHEUS_METRICS_ROUTE=\"$(oc get route --namespace='prometheus' -o go-template --template='{{.spec.host}}' prometheus 2> /dev/null)\"
